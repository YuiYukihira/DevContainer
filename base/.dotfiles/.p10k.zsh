# Generated by Powerlevel10k configuration wizard on 2022-06-20 at 14:50 BST.
...
# sharp tails, 2 lines, solid, full frame, light-ornaments, sparse, fluent,
...
() {
  ...
  typeset -g POWERLEVEL9K_RIGHT_PROMPT_ELEMENTS=(
    ...
    #my_crate                # rust crate name and version
  ...
  function prompt_my_crate() {
    local base=$HOME
    local current_dir={$PWD:/}
    while [[ "$current_dir" != "$base" ]] do
    ...
    if [[ -v target_dir ]]; then
      name=$(grep -m 1 "^name\s*=\s*\".\+\"$" "$target_dir/Cargo.toml" | sed --regexp-extended 's/name\s*=\s*\"(.+)\"/\1/')
      ver=$(grep -m 1 "^version\s*=\s*\".\+\"$" "$target_dir/Cargo.toml" | sed --regexp-extended 's/version\s*=\s*\"(.+)\"/\1/')
      p10k segment -b 1 -f 3 -i '📦' -t "$name v${ver}"
    fi
  }
